locals {
  vpc_outputs = length(var.cluster.providedVPCId) > 0 ? {
    ...
    nat_gateway_count          = local.use_existing_nat_gateways ? length(local.existing_nat_gateway_ids) : module.existing-vpc.nat_gateway_count
    ...
    nat_gateway_ips            = local.use_existing_nat_gateways ? [] : module.vpc.nat_public_ips
    public_subnets_cidr_blocks = module.vpc.public_subnets_cidr_blocks
    nat_gateway_count          = local.use_existing_nat_gateways ? length(local.existing_nat_gateway_ids) : module.vpc.nat_gateway_count
