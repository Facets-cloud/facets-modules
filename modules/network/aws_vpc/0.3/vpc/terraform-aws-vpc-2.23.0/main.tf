...
resource "aws_route_table" "public" {
  count = var.create_vpc && length(var.public_subnets) > 0 && var.network_firewall_enabled == false ? 1 : 0
...
resource "aws_route" "public_internet_gateway_ipv6" {
  count = var.create_vpc && var.enable_ipv6 && length(var.public_subnets) > 0 && var.network_firewall_enabled == false ? 1 : 0
...
resource "aws_route" "private_nat_gateway" {
  count = var.create_vpc && var.enable_nat_gateway && !var.network_firewall_enabled ? local.nat_gateway_count : 0
...
resource "aws_route_table_association" "public" {
  count = var.create_vpc && length(var.public_subnets) > 0 && var.network_firewall_enabled == false ? length(var.public_subnets) : 0
