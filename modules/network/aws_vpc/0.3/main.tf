...
data "aws_nat_gateway" "nat_gw" {
  depends_on = [module.vpc]
  count      = local.network_firewall_enabled ? module.vpc.vpc_details.nat_gateway_count : 0
  state      = "available"
  filter {
    name   = "vpc-id"
    values = [module.vpc.vpc_details.vpc_id]
...
resource "aws_route" "private_nat_gateway" {
  ...
  nat_gateway_id         = data.aws_nat_gateway.nat_gw[count.index].id
...
resource "aws_route_table" "public" {
  ...
  tags = merge(
    local.tags,
    {
      Name = module.vpc.vpc_details.nat_gateway_count > 1 ? "${var.cluster.name}-public-route-table-${var.cluster.azs[count.index]}" : "${var.cluster.name}-public"
    }
  )
}
...
resource "aws_route" "firewall_route" {
  depends_on             = [module.vpc]
  count                  = local.network_firewall_enabled ? length(module.vpc.vpc_details.nat_gateway_ips) : 0
...
resource "aws_route_table" "firewall-rt" {
  ...
  tags = merge(
    local.tags,
    {
      Name = module.vpc.vpc_details.nat_gateway_count > 1 ? "${var.cluster.name}-firewall-${var.cluster.azs[count.index]}" : "${var.cluster.name}-firewall"
    }
  )
}
...
resource "aws_route" "igw_public_firewall_route_multi_az" {
  ...
  vpc_endpoint_id        = module.vpc.vpc_details.nat_gateway_count > 1 ? one([for s in module.network_firewall.0.status[0].sync_states : s.attachment[0].endpoint_id if s.availability_zone == var.cluster.azs[1]]) : (module.network_firewall.0.status[0].sync_states.sync_states[*].attachment[0].endpoint_id)[0]
