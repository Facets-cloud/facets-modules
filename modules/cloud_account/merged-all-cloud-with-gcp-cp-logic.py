...
def fetch_secret(secret_id, cp_cloud):
    if cp_cloud == "aws":
        ...
        # Read values from deployment context file
        context_path = "/sources/deployment_context/deploymentcontext.json"
        with open(context_path, "r", encoding="utf-8") as f:
            context = json.load(f)
        secrets_ctx = context.get("secretsContext", {})

        project = secrets_ctx.get("gcpSecretManagerProjectId")
        region = secrets_ctx.get("secretManagerRegion")
        mode = secrets_ctx.get("gcpSecretManagerMode")

        if not all([project, region, mode]):
            raise ValueError("Missing GCP secret manager config in deployment context")
...
def normalize_output(secret, target_cloud):
    if target_cloud == "AWS":
        return {
            ...
            "session_name": f"capillary-cloud-tf-${{uuid()}}",
