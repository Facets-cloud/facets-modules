...
def build_secret_id(cluster_name, account_id, cp_cloud):
    ...
    """
    if cp_cloud == "GCP":
        return f"{cluster_name}_backend__accounts_{account_id}"
...
def fetch_secret_from_gcp(project_id, secret_id, mode, region):
    ...
    """
    if not all([project_id, secret_id, mode, region]):
        raise ValueError("All parameters (project_id, secret_id, mode, region) are required.")
...
def build_output_object(secret_data, encoded_key):
    ...
    try:
        ...
        # Prefer secret-defined region, fallback to deployment context if missing
...
def main():
    ...
    try:
        ...
        if cp_cloud == "AWS":
            ...
            project_id = get_env_variable("GCP_SECRET_MANAGER_PROJECT_ID", required=True)
            ...
            secret_data = fetch_secret_from_gcp(project_id, secret_id, mode, region)
