...
broker:
  ...
  resources:
    limits:
      ...
      memory: ${local.broker_memory_limits}
    requests:
...
resource "helm_release" "klag-exporter" {
  repository  = lookup(local.kafka_lag_exporter_advanced_config, "repository", "oci://ghcr.io/softwaremill/helm")
  chart       = lookup(local.kafka_lag_exporter_advanced_config, "chart", "klag-exporter")
  version     = lookup(local.kafka_lag_exporter_advanced_config, "chart_version", "0.1.23")
  ...
  values = [
    yamlencode({
      ...
      config = merge(
        {
          exporter = {
            poll_interval = lookup(local.kafka_lag_exporter_advanced_config, "poll_interval", "10s")
            http_port     = 8000
            http_host     = "0.0.0.0"
            granularity   = lookup(local.kafka_lag_exporter_advanced_config, "granularity", "topic")
          }
          "exporter.timestamp_sampling" = {
            enabled = lookup(local.kafka_lag_exporter_advanced_config, "timestamp_sampling_enabled", true)
            mode    = lookup(local.kafka_lag_exporter_advanced_config, "timestamp_sampling_mode", "rate")
          }
          clusters = [
            {
              name              = local.name
              bootstrap_servers = join(",", [for key, value in local.kafka_instances : format("%s:%s", value.host, value.port)])
              group_whitelist   = [".*"]
              group_blacklist   = []
              topic_whitelist   = [".*"]
              topic_blacklist   = ["^__.*"]
        ...
        local.kafka_authenticated ? {
          "clusters.consumer_properties" = {
            ...
            "sasl.username"     = trim(local.username, "\"")
            "sasl.password"     = trim(local.password, "\"")
          }
        } : {},
        lookup(local.kafka_lag_exporter_advanced_config, "config", {})
      )
    }),
    yamlencode(lookup(local.kafka_lag_exporter_advanced_config, "values", {}))
  ]
