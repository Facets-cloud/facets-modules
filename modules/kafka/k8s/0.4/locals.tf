...
locals {
  ...
  kafka_lag_exporter_advanced_config = lookup(local.advanced_config, "kafka_lag_exporter", {})
  controller_dedicated_mode_enabled = lookup(local.controller, "controller_only", "true")
  username                          = local.kafka_authenticated ? "default" : ""
  password                          = local.kafka_authenticated ? coalesce(lookup(local.spec, "password", ""), module.default-password[0].result) : ""
