...
- Renames `metadata.name` to "tools-facets" so the migrated resource deploys in
  parallel with the live nginx `tools` resource (which is left untouched).
- Fills the `inputs` block, defaulting `gateway_api_crd_details.resource_name` to
  "gateway-api-crd" (verify it matches the gateway_api_crd resource in your blueprint).
- Maps the one portable value out of the dropped `advanced` block:
  nginx-ingress `controller.autoscaling.minReplicas` ->
  `spec.data_plane.scaling.min_replicas`.
...
# Resource name for the migrated fabric ingress. Renamed off the source name so the
# new resource deploys ALONGSIDE the live nginx resource instead of replacing it.
# Update this if you re-purpose the script.
HARDCODED_METADATA_NAME = "tools-facets"
...
def convert(input_data, cloud, default_namespace):
    ...
    if "metadata" in input_data:
        ...
        metadata["name"] = HARDCODED_METADATA_NAME
        if original_name and original_name != HARDCODED_METADATA_NAME:
            warn(
                f"RENAMED metadata.name '{original_name}' -> "
                f"'{HARDCODED_METADATA_NAME}' (parallel resource; original untouched)."
            )
    ...
    for key, default_value in DEFAULT_INPUTS.items():
        if key not in inputs:
            ...
            if key == "gateway_api_crd_details":
                warn(f"ADDED: '{key}' with default resource_name 'gateway-api-crd'.")
                warn(
                    "       Verify this matches the gateway_api_crd resource name in "
                    "your blueprint before deploying."
                )
            else:
    ...
    # One value is portable and gets mapped across (autoscaling.minReplicas).
    advanced_min_replicas = None
    ...
    if advanced:
        ...
        autoscaling = (
            ((nic.get("values", {}) or {}).get("controller", {}) or {}).get(
                "autoscaling", {}
            )
            or {}
        )
        if "minReplicas" in autoscaling:
            advanced_min_replicas = autoscaling["minReplicas"]
            warn(
                f"  advanced...controller.autoscaling.minReplicas="
                f"{advanced_min_replicas} -> mapped to "
                f"spec.data_plane.scaling.min_replicas."
            )
    ...
    # Merge the mapped autoscaling.minReplicas into data_plane (input spec.data_plane,
    # if any, wins — only fill the gap).
    if advanced_min_replicas is not None:
        data_plane = dict(out_spec.get("data_plane", {}) or {})
        scaling = dict(data_plane.get("scaling", {}) or {})
        if "min_replicas" not in scaling:
            scaling["min_replicas"] = advanced_min_replicas
            data_plane["scaling"] = scaling
            out_spec["data_plane"] = data_plane
