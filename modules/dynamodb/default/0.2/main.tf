locals {
  ...
  }, lookup(local.aws_dynamodb, "tags", {}))
}

module "dynamodb" {
  source = "./terraform-aws-dynamodb-table-2.0.0"

  name                               = "${var.instance_name}-${var.cluster.name}"
...
resource "aws_iam_policy" "read_only_policy" {
  name = "${var.cluster.name}-${module.dynamodb.dynamodb_table_id}_ro"
  tags = {
    "Cluster"            = var.cluster.name
    "FacetsControlPlane" = var.cc_metadata.cc_host
    "FacetsClusterName"  = var.cluster.name
...
resource "aws_iam_policy" "read_write_policy" {
  name = "${var.cluster.name}-${module.dynamodb.dynamodb_table_id}_rw"
  tags = {
    "Cluster"            = var.cluster.name
    "FacetsControlPlane" = var.cc_metadata.cc_host
    "FacetsClusterName"  = var.cluster.name
